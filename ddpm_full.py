...
class TransformerEncoderSA(nn.Module):
    ...
    def forward(
        ...
        attention_value = self.mem_efficient_attn(
            ...
            mask=contextual_text_mask_embedding
        )
...
class UNetConditional(nn.Module):
    ...
    def forward(
            ...
            contextual_text_mask_embedding: torch.Tensor,
    ) -> torch.Tensor:
        ...
        if conditional_embedding is not None:
            ...
            contextual_text_mask_embedding.squeeze_(dim=1)

        x1 = self.input_conv(x)

        x2 = self.down1(x1, t)
        x2 = self.sa1(x2, contextual_text_embedding, contextual_text_mask_embedding)
        x3 = self.down2(x2, t)
        x3 = self.sa2(x3, contextual_text_embedding, contextual_text_mask_embedding)
        x4 = self.down3(x3, t)
        x4 = self.sa3(x4, contextual_text_embedding, contextual_text_mask_embedding)
